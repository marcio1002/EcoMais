...
$("#btnRecoverPwd").click(function() {
    ...
    const option = {
        ...
        success: (res) => {
            if (res) 
                if(res.error) return $(".alert").addClass("alert-danger").text("Não foi possível atualizar a senha!")
                ...
                $("#container-btnRecoverPwd").append(`<a class="btn btn-lg btn-block btn-success" href="${BASE_URL}/login">Voltar para login</a>`)
                $(this).remove()
